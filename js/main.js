...
const METOFFICE_FORECASTS_API = "https://api-metoffice.apiconnect.ibmcloud.com/v1/forecasts/point/"
...
const getWeatherFromOpenMeteo = () => {
  ...
  fetch(api)
    ...
    .then(data => {
      ...
      tbody.appendChild(makeTr("td", max))
      tbody.appendChild(makeTr("td", min))
    })
    .catch(error => {
...
const getWeatherFromMetOffice = async () => {
  const latitude = document.querySelector('#latitude').value
  const longitude = document.querySelector('#longitude').value
  const url = `${METOFFICE_SITSPECIFIC_API}hourly?latitude=${latitude}&longitude=${longitude}`
  ...
  for (const series of data.features[0].properties.timeSeries) {
    console.log(`${series.feelsLikeTemperature}, ${series.probOfPrecipitation}`)
  }
  // console.log(data)
}
